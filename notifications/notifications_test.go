...
func TestNotifyUser(t *testing.T) {
	...
	_, err := notifyUser(nil, userID, fc_badsend, testutil.MockStorage{})
	if err == nil {
		t.Errorf("Should have returned an error")
	}

	_, err = notifyUser(nil, userID, fc, testutil.MockStorage{})
	if err != nil {
...
func (m mockPollComm) RequestNotifications(host *connect.Host) (*pb.UserIdList, error) {
	return &pb.UserIdList{
		IDs: [][]byte{id.ZeroUser.Marshal()},
	}, nil
}
func (m mockPollComm) GetHost(hostId *id.ID) (*connect.Host, bool) {
	return &connect.Host{}, true
}
func (m mockPollComm) AddHost(id *id.ID, address string, cert []byte, disableTimeout, enableAuth bool) (host *connect.Host, err error) {
	return nil, nil
}
func (m mockPollComm) RequestNdf(host *connect.Host, message *pb.NDFHash) (*pb.NDF, error) {
	return nil, nil
}
func (m mockPollComm) RetrieveNdf(currentDef *ndf.NetworkDefinition) (*ndf.NetworkDefinition, error) {
	return nil, nil
}
...
func (m mockPollErrComm) AddHost(id *id.ID, address string, cert []byte, disableTimeout, enableAuth bool) (host *connect.Host, err error) {
	return nil, nil
}
...
func TestImpl_RegisterForNotifications(t *testing.T) {
	...
	userID := id.NewIdFromString("test", id.User, t)
	host, err := connect.NewHost(userID, "0.0.0.0:420", crt, false, false)
	if err != nil {
